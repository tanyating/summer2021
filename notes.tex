...
\begin{itemize}
    ...
    \item Methods:
    \begin{enumerate}
        \item $\min_{t, R} \lVert \mathbf{y}- \mathbf{a}_{t,R}\rVert^2$
        \item $\max_{t,R} \langle \hat{y}, \hat{a_{t,R}} \rangle$, where $\hat{v} = \frac{1}{c}(\mathbf{v} - v_m)$ and $v_m = \frac{\sum_{i=1}^N v_i}{N}$, $c = \lVert \mathbf{v}- v_m\rVert$
        \item Let $f(t) = \max_R \langle \hat{y}, \hat{a_{t,R}} \rangle$. Pick a threshold $\tau$ based on false positive/negative rates (penalty).\\
        If $f(\widetilde{t}) > \tau$ and $\widetilde{R} = \argmax_R \langle \hat{y}, \hat{a_{\widetilde{t},R}} \rangle$, then $\widetilde{t}$, $\widetilde{R}$ is a candidate.
        \item Let $g(t) = {mean}_R \langle \hat{y}, \hat{a_{t,R}} \rangle$, $h(t) = {variance}_R  \langle \hat{y}, \hat{a_{t,R}} \rangle$, and $j(t) = \frac{f(t)-g(t)}{\sqrt{h(t)}}$. \\
        If $j(\widetilde{t}) > \tau$ and $\widetilde{R} = \argmax_R \langle \hat{y}, \hat{a_{\widetilde{t},R}} \rangle$, then $\widetilde{t}$, $\widetilde{R}$ is a candidate.
    \end{enumerate}
\end{itemize}
...
LDA
\begin{itemize}
    \item Determine decision boundary (in any dimension d, $\mathbf{x} \in \mathbf{R}^d$) with prior and penalty matrix.
\begin{enumerate}
    ...
    \item False positive rate and false negative rate given decision point $b$ are given in equations \ref{eq:1} and \ref{eq:2}.
\end{enumerate}
