...
\begin{itemize}
    ...
    \item Given molecule $mol$, generate template $A \in \mathbf{R}^{(N-p+1) \times 4 \times N}$ such that $A(i,j,:) \in \mathbf{R}^N = a_{t=i-1, R=(j-1)\frac{\pi}{2}}$ represents a projected signal with certain translation ($i \in {1,..,N-p+1}$) and rotation ($j \in {1,..,4}$). (We can reshape $A$ into $AA \in \mathbf{R}^{N_c \times N}$ so that each row represents a configuration, ie. a projected signal without noise).
