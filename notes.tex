...
\begin{itemize}
    \item Assume molecule with discrete pixels ($m \in \mathbf{R}^{p \times q}$ and $p>q$ in 2-D), $N$ number of grids, 4 rotations $R \in \{0, \frac{\pi}{2}, \pi, \frac{3\pi}{2}\}$, and $N-p+1$ translations $t \in {0,1,2,...,N-p}$. For each instance (1-D projection) $\mathbf{y} \in \mathbf{R}^N$, each component $y^i$ of $\mathbf{y}$ represents the sum along the ith column. In total, there are $N_c = 4\times(N-p+1)$ configurations (set of translations and rotations) for signals (ie. molecule detected). 
    \item Given molecule $m$, generate template $A \in \mathbf{R}^{N_c \times N}$ such that each row $A(k,:) \in \mathbf{R}^N = a_{t=i, R=(j-1)\frac{\pi}{2}}$ represents a projected signal with certain translation ($i \in {0,..,N-p}$) and rotation ($j \in {1,..,4}$). (We can reshape $A$ into $AA \in \mathbf{R}^{N_c \times N}$ so that each row represents a configuration, ie. a projected signal without noise).
