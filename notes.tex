...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item Observation: 
        \begin{itemize}
            \item The intersection of the 1st column and 1st row $C[0,0]=tn$ is the true negative rate (noise detected as noise). 
            \item Regardless of 1st entry, the rest of the first row is roughly the same $a=fp_noise$, which indicates some false positive rate for the noise (noise detected as signal/molecule).
            \item Regardless of 1st entry, the rest of the first row is roughly the same $a=fp_noise$, which indicates some false positive rate for the noise (noise detected as signal/molecule).
        \end{itemize}
